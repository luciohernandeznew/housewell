...
import {fetchUserSettings} from "../../slices/userSettings";
import { selectUnreadMsgNotifications } from "../../slices/notifications";
import { useDevice } from "../../contexts/DeviceContext";
import MobileNavOptionsButton from "./MobileNavOptionsButton";
...
const AuthNav: React.FC = () => {
    ...
    const isMobile = windowSize.width < 938;
    const unauthedRoutes = ['/buy/map', '/property-listing',];
    const isunauthedAllowed = unauthedRoutes.some(route => router.pathname.includes(route))
    useEffect(() => {
        if (isunauthedAllowed) {
            return;
        }
        if (userReducer.status === 'failed') {
            if (userReducer.error === 'EMAIL_NOT_VERIFIED') {
                router.pathname !== '/verify-email' ? router.push('/verify-email') : null;
            } else {
                router.push('/login');
            }
        }
    ...
    return <NavContainer mobile={isMobile}>
        {(!user && isunauthedAllowed) ? <UnauthedNav/> : <>
        {isMobile ? <>
        {user ? <div style={{width:'100%', display:'flex', alignItems:'center', marginLeft:'5%', marginRight: '5%'}}>
            <Link style={{}} href={user?.onboardingStep || '/'}>
                <MobileIconSvg/>
            </Link>
            { user.firstName &&
            <div style={{justifyContent:'center', width:'100%', display: "flex", alignItems: "center" }}>
                <Link href="/dashboard">
                    <LinkText size={"20"} weight={"medium"} selected={sellIsSelected}>Sell</LinkText>
                </Link>
                <span style={{ marginRight: "28px" }} />
                {user.transactionSideType !== 'seller' && <Link href="/buy">
                    <LinkText size={"20"} weight={"medium"}  selected={router.pathname.includes("buy")}>Buy</LinkText>
                </Link>}
            </div>
            
            }
        {user.firstName && <MobileNavOptionsButton/>}
            
            </div>
        :<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center",  width: "100%" }}>
            <Link style={{marginLeft:"5%", marginRight: "auto" }} href={user?.onboardingStep || '/'}>
            <MobileIconSvg/>
            </Link>
            {userReducer.status === "loading" ?
                <LoadingSpinner style={{ zIndex: "-1" }} />
                :
                router.pathname !== '/verify-email' && 
                <div style={{ marginRight: "64px", display: "flex", minWidth: "180px", width: "180px" }} >
                    <SecondaryButton style={{ marginRight: "24px", flexBasis: "50%", padding: 0 }} size='medium' isLight text={"Sign Up"} onClick={() => router.push('/signup')} />
                    <SecondaryButton style={{ flexBasis: "50%", padding: 0 }} size='medium' text={"Log In"} onClick={() => router.push('/login')} />
                </div>
            }
        </div> }
        </> 
        :
        <>{user ? <div style={{width:'100%', display:'flex', alignItems:'center'}}>
            <Link style={{marginLeft:"64px", marginRight: 'auto'}} href={user?.onboardingStep || '/'}>
                <IconSvg/>
            </Link>
            { user.firstName &&
            <div style={{justifyContent:'space-between', paddingLeft:'70px', display: "flex", alignItems: "center" }}>
                <Link href="/dashboard">
                    <LinkText size={"20"} weight={"medium"} style={{marginRight: "32px"}} selected={sellIsSelected}>Sell</LinkText>
                </Link>
                {user.transactionSideType !== 'seller' && <Link href="/buy">
                    <LinkText size={"20"} weight={"medium"} style={{marginRight: "32px"}} selected={router.pathname.includes("buy")}>Buy</LinkText>
                </Link>}
                <Link href="/messages">
                    <div style={{ display: "flex", alignItems: "center" }}>
                        <LinkText size={"20"} weight={"medium"} style={{}} selected={router.pathname.includes("messages")}>Messages</LinkText>
            ...
            { user.firstName &&
            <div style={{display: "flex", alignItems: "center", marginLeft: 'auto', marginRight: "64px"}}>
                <NotificationBox />
            ...
            {userReducer.status === "loading" ?
                ...
                :
                router.pathname !== '/verify-email' && 
                <div style={{ marginRight: "64px", display: "flex", minWidth: "240px", width: "320px" }} >
                    <SecondaryButton style={{ marginRight: "24px", flexBasis: "50%", padding: 0 }} size='medium' isLight text={"Sign Up"} onClick={() => router.push('/signup')} />
                    <SecondaryButton style={{ flexBasis: "50%", padding: 0 }} size='medium' text={"Log In"} onClick={() => router.push('/login')} />
                </div>
            }
