...
const SellerDisclosure7 = () => {
    const router = useRouter();
    const [selectedAppAge, setSelectedAppAge] = useState<any>(null);
    const [selectedSellerOwner, setSelectedSellerOwner] = useState<any>(null);
    const [selectedRoof, setSelectedRoof] = useState<any>(null);

    const handleNextSubmit = () => {
        router.push("/onboarding/property/part-4/seller-disclosure-8");
    };
    return (
        <OnboardingScreenFrame nextOnClick={handleNextSubmit}>
            <H5 style={{ marginBottom: "24px" }}>
                Seller&apos;s Disclosure: ROOFS, GUTTERS, and DOWNSPOUTS
            </H5>{" "}
            ...
            <StyledInput
                type="text"
                min={0}
                value={selectedAppAge}
                onChange={(e) => {
                    const numericValue = e.target.value.replace(/[^0-9]/g, "");
                    setSelectedAppAge(numericValue);
                }}
                required
            />
            <MintParagraph
                ...
                style={{ marginTop: "24px", marginBottom: "24px" }}
            >
                (b) Has any part of the roof been repaired during Seller’s
                ownership?
            </MintParagraph>
            <MultipleChoiceParent
                ...
                selectedIndex={selectedSellerOwner}
                onSelection={(index) => setSelectedSellerOwner(index)}
            />
            <MintParagraph
                size="24"
                weight="regular"
                style={{ marginTop: "24px", marginBottom: "24px" }}
            >
                (c) Are there any roof leaks or other problems with the roof,
                roof flashing, gutters, or downspouts?
            </MintParagraph>
            <MultipleChoiceParent
                buttonHeight="64px"
                fontSize="20"
                ...
                style={{ margin: "5px 0 36px 0" }}
                choices={disclosureEnumOptions}
                selectedIndex={selectedRoof}
                onSelection={(index) => setSelectedRoof(index)}
            />
